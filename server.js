...
app.post("/api/projects/delete-page", async (req, res) => {
  const { page, rowsPerPage } = req.body;
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM projects WHERE sort_order > $1 AND sort_order <= $2",
      [start, end]
    );
    ...
    res.status(500).json({ error: "Failed to delete project page" });
  }
});
...
app.post("/api/pipeline", async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO pipe_in_liner (description, status, remarks, sort_order)
     VALUES ($1,$2,$3,$4) RETURNING *`,
    [description, status, remarks, nextOrder]
  );
    ...
    for (let i = 1; i <= rowsPerPage; i++) {
      await pool.query(
        `INSERT INTO pipe_in_liner (
          description, status, remarks, sort_order
        ) VALUES ('', '', '', $1)`,
        [insertIndex + i]
      );
    }
...
app.post("/api/pipeline/delete-page", async (req, res) => {
  const { page, rowsPerPage } = req.body;
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM pipe_in_liner WHERE sort_order > $1 AND sort_order <= $2",
      [start, end]
    );
    ...
    await pool.query("ROLLBACK");
    console.error("Delete pipeline page error:", err);
    res.status(500).json({ error: "Failed to delete pipeline page" });
  }
});
...
app.post("/api/deployment/delete-page", async (req, res) => {
  const { page, rowsPerPage } = req.body;
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM deployment WHERE sort_order > $1 AND sort_order <= $2",
      [start, end]
    );
    ...
    await pool.query("ROLLBACK");
    console.error("Delete deployment page error:", err);
    res.status(500).json({ error: "Failed to delete deployment page" });
  }
});
...
app.post("/api/application/delete-page", async (req, res) => {
  const { page, rowsPerPage } = req.body;
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM application WHERE sort_order > $1 AND sort_order <= $2",
      [start, end]
    );
    ...
    await pool.query("ROLLBACK");
    console.error("Delete application page error:", err);
    res.status(500).json({ error: "Failed to delete application page" });
  }
});

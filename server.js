...
app.post("/api/login", (req, res) => {
  ...
  if (
  (username === process.env.LOGIN_USER_1 && password === process.env.LOGIN_PASS_1) ||
  (username === process.env.LOGIN_USER_2 && password === process.env.LOGIN_PASS_2)
) {
    const now = Date.now();
    const existing = onlineUsers[username];

    if (
      existing &&
      now - existing.lastSeen <= 20000 &&
      existing.sessionId !== sessionId
    ) {
      return res.status(409).json({
        error: "This account is already logged in on another device or tab"
      });
    }
...
app.delete("/api/pipeline/:id", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM pipe_in_liner WHERE id=$1",
      [id]
    );
...
app.post("/api/online-users", (req, res) => {
  ...
  if (!username || !sessionId) {
    return res.status(400).json({
      error: "Missing username or sessionId"
    });
  }
  ...
  if (
    ...
    return res.status(409).json({
      error: "Account active elsewhere"
    });
  }
...
app.post("/api/logout-session", (req, res) => {
  ...
  if (
    ...
    sessionId &&
    onlineUsers[username] &&
    onlineUsers[username].sessionId === sessionId
  ) {
    delete onlineUsers[username];
  }
...
ensureProjectGroupColumns()
  ...
  .catch((err) => {
    console.log("Database unavailable, running in offline mode");

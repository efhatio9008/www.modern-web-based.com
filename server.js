...
async function cleanupOldActivityLogs() {
  await pool.query("DELETE FROM activity_logs WHERE created_at < NOW() - INTERVAL '72 hours'");
}
...
app.post("/api/activity/login", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO activity_logs (username, login_time, modules)
       VALUES ($1, NOW(), ARRAY[]::TEXT[])
       RETURNING *`,
      [username || "Unknown"]
    );
...
app.put("/api/activity/modules/:id", async (req, res) => {
  try {
    await cleanupOldActivityLogs();
    const { id } = req.params;
    ...
    console.error("UPDATE activity modules error:", err);
    res.status(500).json({ error: "Failed to update activity modules" });
  }
});

app.put("/api/activity/logout/:id", async (req, res) => {
  try {
    await cleanupOldActivityLogs();
    const { id } = req.params;
    const modules = Array.isArray(req.body.modules) ? req.body.modules : [];
    ...
    console.error("UPDATE activity logout error:", err);
...
/* =========================
   START SERVER
========================= */
Promise.all([ensureProjectGroupColumns(), ensureActivityLogTable()])
  .then(() => {

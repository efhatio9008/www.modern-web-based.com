  ...
  return `
    <tr class="timeline-day-row">
      ...
      ${months.map((m, monthIndex) => {
        ...
        return `<td class="timeline-month-cell ${editable ? "" : "timeline-disabled-cell"}" ${editable ? "contenteditable=\"true\"" : "contenteditable=\"false\""} data-fill="${editable ? "true" : "false"}" data-module="timeline" data-id="${row.id}" data-field="${monthIndex}" ${disabledText} ${editable ? `onblur="updateTimelineCell(${row.id}, ${monthIndex}, this.innerText)"` : ""}>${editable ? (m || "") : "—"}</td>`;
      }).join("")}
      <td contenteditable="true" data-fill="true" data-module="timeline" data-id="${row.id}" data-field="status" onblur="updateTimelineStatus(${row.id}, this.innerText)">${row.status || ""}</td>
    </tr>`;
}
...
function activeSelectionInsideCell(cell) {
...
function cleanWrapperContent(cell) {
  if (!cell) return "";
  return cell.innerHTML.trim();
}

function wrapWholeCell(cell, styleText) {
  const current = cleanWrapperContent(cell);
...
async function applyFormatAlign(module, alignment) {
  ...
  wrapWholeCell(cell, `text-align:${alignment};`);
...
async function applyFormatSize(module, size) {
  ...
  if (activeSelectionInsideCell(cell)) {
    ...
    wrapWholeCell(cell, `font-size:${size};`);
  }
...
async function applyFormatColor(module, color) {
  ...
  if (activeSelectionInsideCell(cell)) {
    ...
    wrapWholeCell(cell, `color:${color};`);
  }

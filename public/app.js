...
function renderApplication() {
  ...
  let displayData = applicationData.map((a, index) => ({
    ...a,
    displayId: index + 1
  }));
...
function searchApplication() {
  ...
  const found = applicationData
    .map((a, index) => ({ ...a, displayId: index + 1 }))
    .filter(a => a.displayId === id);
...
async function deletePage(module) {
  ...
  if (module === "dashboard") {
    rowsPerPage = 15;
    page = currentPage;
    apiPath = "/api/projects/delete-page";
  }

  if (module === "pipeline") {
    rowsPerPage = 20;
    page = pipelinePage;
    apiPath = "/api/pipeline/delete-page";
  }

  if (module === "deployment") {
    rowsPerPage = 20;
    page = deploymentPage;
    apiPath = "/api/deployment/delete-page";
  }

  if (module === "application") {
    rowsPerPage = 20;
    page = applicationPage;
    apiPath = "/api/application/delete-page";
  }
  ...
  if (module === "deployment") {
    deploymentPage = Math.max(1, deploymentPage - 1);
    loadDeployment();
  }

  if (module === "application") {
    applicationPage = Math.max(1, applicationPage - 1);
    loadApplication();
  }
...
function calculateModule(data, module) {
  ...
  if (module === "dashboard") rowsPerPage = 15;
  if (module === "pipeline") rowsPerPage = 20;
  if (module === "deployment") rowsPerPage = 20;
  if (module === "application") rowsPerPage = 20;

  data.forEach((row, index) => {
    const rowNo = index + 1;

    // 🔥 找这行属于第几页
    const page = Math.ceil(rowNo / rowsPerPage);

    // 🔥 拿对应 page 的 title
    const key = `title-${module}-page-${page}`;
    const pageTitle = localStorage.getItem(key)?.trim();
    ...
    for (let k in row) {
      if (k === "id" || k === "sort_order" || k === "displayId") continue;
    ...
    if (result[pageTitle].start === null) {
      result[pageTitle].start = rowNo;
    }

    result[pageTitle].end = rowNo;

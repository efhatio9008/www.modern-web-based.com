...
function goDashboard() {
  ...
  document.getElementById("pipelinePage").style.display = "none"; // 🔥 加这个
}
...
function searchPipeline() {
  ...
  const found = pipelineData.filter(p => p.id === id);
    ...
    table.innerHTML += `
      <tr>
       <td>${row.id}</td>
        ...
        <td><button onclick="deletePipeline(${row.id})">Delete</button></td>
      </tr>
    `;
  });
}

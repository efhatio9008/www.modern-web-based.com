...
function cloneTableForExport(tableElement) {
  ...
  clone.querySelectorAll('td, th').forEach(cell => {
    ...
    cell.style.fontSize = computed.fontSize;
...
function exportGroup(module, groupTitle) {
  ...
  allRows.forEach(row => {
    ...
    if (headerCell) {
      ...
      if (insideTarget) {
        tempTable.appendChild(cloneTableForExport(row));
      }
  ...
  downloadExcelHtml(`${module}_${safeFilename(groupTitle)}.xls`, html);
}

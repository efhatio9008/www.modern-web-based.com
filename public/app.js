...
function calculateModule(data, fieldName) {
  let result = {};
  let total = 0;

  let firstRow = null; // ✅ 新增
  let lastRow = null;  // ✅ 新增

  data.forEach((row, index) => {
    const text = (row[fieldName] || "").trim();
    ...
    result[type].count++;
    result[type].end = rowNo;
  });

  return { result, total, firstRow, lastRow }; // ✅ 多 return 两个
}
...
async function runCalculation() {
  ...
  html += buildTable(
    "Pipeline",
    calculateModule(pipelineData, "description")
  );

  html += buildTable(
    "Deployment",
    calculateModule(deploymentData, "deployment_type")
  );

  html += buildTable(
    "Application",
    calculateModule(applicationData, "project_description")
  );

  html += buildTable(
    "Dashboard",
    calculateModule(allProjects, "project_description")
  );

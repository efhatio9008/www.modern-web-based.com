  ...
  pipelineData.forEach((row, i) => {
    table.innerHTML += `
      <tr>
        ...
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'description', this.innerText)">${row.description || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'status', this.innerText)">${row.status || ""}</td>

        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'presentation', this.innerText)">${row.presentation || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'commercial', this.innerText)">${row.commercial || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'technical', this.innerText)">${row.technical || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'site_visit', this.innerText)">${row.site_visit || ""}</td>

        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'received_orders', this.innerText)">${row.received_orders || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'issue_po', this.innerText)">${row.issue_po || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'transport', this.innerText)">${row.transport || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'lead_time', this.innerText)">${row.lead_time || ""}</td>

        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'etb', this.innerText)">${row.etb || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'eta', this.innerText)">${row.eta || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'transport_site', this.innerText)">${row.transport_site || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'do_acceptance', this.innerText)">${row.do_acceptance || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'install_start', this.innerText)">${row.install_start || ""}</td>
        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'project_end', this.innerText)">${row.project_end || ""}</td>

        <td contenteditable="true" onblur="updatePipeline(${row.id}, 'remarks', this.innerText)">${row.remarks || ""}</td>

        <td>
          <button onclick="deletePipeline(${row.id})">Delete</button>
        </td>
      </tr>
    `;
  });
...
async function deletePipeline(id) {
  ...
  pipelineData = pipelineData.filter(r => r.id !== id);
  renderPipeline();
}
...
function exportPipeline() {
  ...
  const headers = [
    "Project","Status","Presentation","Commercial","Technical",
    "Site Visit","Orders","Issue PO","Transport","Lead Time",
    "ETB","ETA","Transport Site","DO Acceptance","Install Start","Project End","Remarks"
  ];

  csv.push(headers.join(","));

   // 🔥 就放这里
  const sortedData = [...pipelineData].sort((a, b) => b.id - a.id);

   // 🔥 用 sortedData，不是 pipelineData
  sortedData.forEach(row => {
    csv.push([
      row.description || "",
      row.status || "",

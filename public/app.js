    ...
    table.innerHTML += `
      <tr>
        ${row.months.map((m, monthIndex) => `
          <td contenteditable="true" onblur="updateTimelineCell(${realIndex}, ${monthIndex}, this.innerText)">${m}</td>
        `).join("")}

        <td contenteditable="true" onblur="updateTimelineStatus(${realIndex}, this.innerText)">${row.status}</td>

        <td>
          <button onclick="deleteTimelineRow(${realIndex})">Delete</button>
        </td>
      </tr>
    `;
  });
...
function updateTimelineCell(rowIndex, monthIndex, value) {
  timelineData[rowIndex].months[monthIndex] = value.trim();
}

function updateTimelineStatus(rowIndex, value) {
  timelineData[rowIndex].status = value.trim();
}

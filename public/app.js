...
/* =========================
   ...
   Widths are saved in browser localStorage and survive refresh/login/redeploy on the same browser.
========================= */
...
function saveColumnWidth(module, columnIndex, width) {
  ...
  widths[columnIndex] = Math.max(40, Math.round(width));
...
function setColumnWidth(table, columnIndex, width) {
  ...
  const px = `${Math.max(40, Math.round(width))}px`;
  ...
  table.querySelectorAll("tr").forEach(row => {
    ...
    /*
      Important:
      Do NOT set maxWidth to the same px value.
      That was causing the next columns to feel "locked" after resizing
      Project / Project Description / Deployment Type.
      Width + minWidth is enough for persistence, while maxWidth stays open.
    */
    cell.style.width = px;
...
function applySavedColumnWidths(module) {
  const table = getResizableTable(module);
  if (!table) return;
  const widths = loadColumnWidths(module);
  Object.entries(widths).forEach(([columnIndex, width]) => {
    setColumnWidth(table, Number(columnIndex), Number(width));
  });
}

...
function buildTable(title, dataObj) {
  ...
  for (const type in result) {
    const r = result[type];
    html += `<tr><td>${escHtml(type)}</td><td>${r.count}</td><td>${r.start} - ${r.end}</td></tr>`;
  }

  html += `<tr style="font-weight:bold; background:#f2f2f2;"><td>Total</td><td>${total}</td><td>NA</td></tr></table>`;
  return html;
}

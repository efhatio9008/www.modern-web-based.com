...
// =========================
// 🔥 Excel Drag Fill (Timeline)
// =========================

function startDrag(cell) {
  isDragging = true;
  startCell = cell;
  startValue = cell.innerText.trim();
}
...
function dragFill(cell) {
  ...
  if (!isNaN(currentValue)) {
    ...
    startValue = cell.innerText;
    return;
  }

  // 2️⃣ RM (RM1 → RM2 → RM3)
  if (/^RM\d+$/.test(currentValue)) {
    let num = Number(currentValue.replace("RM", ""));
    cell.innerText = "RM" + (num + 1);
    startValue = cell.innerText;
    return;
  }

  // 3️⃣ 日期 (2026-01-01 → +1 day)

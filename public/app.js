...
function showOnlyPage(pageId) {
  ["dashboardPage", "timelinePage", "pipelinePage", "deploymentPage", "applicationPage", "calculatePage", "activityPage"].forEach(id => {
  ...
  return {
    dashboard: {
      ...
      blankBody: group => ({
        ...
        cost_proposal: "", si_report: "", ccc_readiness_manpower: "", procurement_material: "",
...
function normalizeContentForCalculate(value) {
  if (value === null || value === undefined) return "";

  let raw = String(value);
  if (!raw) return "";

  // Contenteditable cells can look empty but still save hidden HTML such as
  // <br>, <div><br></div>, &nbsp; or zero-width spaces. Calculation should
  // only count real visible text, not these empty HTML leftovers.
  raw = raw
    .replace(/<br\s*\/?\s*>/gi, " ")
    .replace(/&nbsp;/gi, " ")
    .replace(/ /g, " ")
...
function rowHasContent(row, module) {
  if (module === "timeline") {
    return (row.months || []).some(v => normalizeContentForCalculate(v) !== "") ||
      normalizeContentForCalculate(row.status) !== "";
  }

  const skip = new Set(["id", "sort_order", "displayId", "project_group", "groupTitle", "year"]);
  return Object.keys(row).some(key => !skip.has(key) && normalizeContentForCalculate(row[key]) !== "");
}
    ...
    <tr class="group-title-row ${deleting ? "delete-mode-active" : ""}">
      <td colspan="99">
        <div class="group-header-box">
          ...
          <button class="group-email-btn" onclick='openEmailModal(${moduleArg}, ${groupArg})'>Send Email</button>
        </div>
      </td>
    </tr>
  `;
...
function renderDashboardRow(p) {
  ...
  return `
    <tr>
      ...
      <td contenteditable="true" data-fill="true" data-module="dashboard" data-id="${p.id}" data-field="si_report" id="si_report-${p.id}" onblur="autoSave(${p.id})">${(p.si_report || "")}</td>
  ...
  let html = `
    <div class="calc-module-card">
      ...
      <table class="calc-table" border="1">
        ...
        <tr><th>Type</th><th>${title === "Pipeline" ? "Project Description" : (title === "Deployment" || title === "Application" ? "Task" : "Projects")}</th><th>Range</th></tr>
  `;
...
/* =========================
   CELL ATTACHMENT TOOLBAR
   Adds/removes an attachment box inside the currently selected editable cell.
   Uses existing text fields, so no database schema change is needed.
========================= */
function attachmentHtml(filename = "") {
  const safeName = escHtml(filename);
  const fileBox = filename
    ? `<div class="cell-attachment-file">📎 <a href="/uploads/${safeName}" target="_blank" rel="noopener">${safeName}</a><button type="button" class="cell-attachment-remove-file" title="Remove file">❌</button></div>`
    : `<div class="cell-attachment-empty">No file attached</div>`;

  return `<div class="cell-attachment-box" data-cell-attachment="1" data-filename="${safeName}" contenteditable="false">${fileBox}<div class="cell-attachment-upload"><input type="file" class="cell-attachment-input"><button type="button" class="cell-attachment-upload-btn">Upload</button></div></div>`;
}

function getAttachmentBox(cell) {
  return cell ? cell.querySelector(".cell-attachment-box[data-cell-attachment='1']") : null;
}

async function toggleCellAttachment(module) {
  const cell = getActiveFormatCell(module);
  if (!cell) return;

  const existing = getAttachmentBox(cell);

  if (existing) {
    if (!confirm("Remove attachment function from this cell?")) return;
    existing.remove();
    if (!cell.textContent.trim() && !cell.querySelector(".cell-attachment-box")) cell.innerHTML = "";
    await saveFormattedCell(cell);
    refreshAttachmentToolbarState(module);
    return;
  }

  if (!confirm("Add attachment function to this cell?")) return;

  const current = cell.innerHTML.trim();
  cell.innerHTML = current ? `${current}<br>${attachmentHtml()}` : attachmentHtml();
  await saveFormattedCell(cell);
  refreshAttachmentToolbarState(module);
}

async function uploadAttachmentForCell(box) {
  const cell = box?.closest("td[contenteditable='true'][data-fill='true']");
  const input = box?.querySelector(".cell-attachment-input");

  if (!cell || !input || !input.files.length) {
    alert("Please choose a file first");
    return;
  }
  ...
  const res = await fetch("/api/cell-attachment-upload", {
    method: "POST",
    body: formData
  });
  ...
  box.outerHTML = attachmentHtml(filename);
  await saveFormattedCell(cell);
  refreshAttachmentToolbarState(cell.dataset.module);
}

async function removeAttachmentFileFromCell(box) {
  const cell = box?.closest("td[contenteditable='true'][data-fill='true']");
  if (!cell) return;
  if (!confirm("Remove this attached file from this cell?")) return;

  box.outerHTML = attachmentHtml();
  await saveFormattedCell(cell);
  refreshAttachmentToolbarState(cell.dataset.module);
}

function refreshAttachmentToolbarState(module) {
  const cell = activeFormatCellByModule[module];
  document.querySelectorAll(`.format-toolbar[data-format-module='${module}'] .attachment-format-btn`).forEach(button => {
    button.classList.toggle("attachment-format-btn-active", Boolean(getAttachmentBox(cell)));
  });
}

document.addEventListener("click", event => {
  const uploadBtn = event.target.closest(".cell-attachment-upload-btn");
  if (uploadBtn) {
    event.preventDefault();
    event.stopPropagation();
    uploadAttachmentForCell(uploadBtn.closest(".cell-attachment-box"));
    return;
  }

  const removeBtn = event.target.closest(".cell-attachment-remove-file");
  if (removeBtn) {
    event.preventDefault();
    event.stopPropagation();
    removeAttachmentFileFromCell(removeBtn.closest(".cell-attachment-box"));
  }
});

document.addEventListener("mousedown", event => {
  if (event.target.closest(".cell-attachment-box")) {
    event.stopPropagation();
  }
});
...
function renderSharedControlSlots() {
  ...
  document.querySelectorAll(".format-toolbar-slot").forEach(slot => {
    ...
    slot.innerHTML = `
      ...
      <button type="button" class="format-btn attachment-format-btn" title="Attachment" onclick="toggleCellAttachment('${module}')">📎</button>
...
/* =========================
   TOP HORIZONTAL TABLE SCROLL
   Adds a synced horizontal scrollbar above each table-wrapper.
   Bottom native scrollbar stays unchanged.
========================= */
function ensureTopHorizontalScrollbars() {
  document.querySelectorAll(".table-wrapper").forEach((wrapper, index) => {
    ...
    if (!topScroll || !topScroll.classList.contains("table-scroll-top")) {
      ...
      topScroll.dataset.scrollIndex = String(index);
      topScroll.innerHTML = "<div class='table-scroll-top-inner'></div>";
    ...
    function refreshTopScrollWidth() {
      inner.style.width = `${wrapper.scrollWidth}px`;
      topScroll.style.display = wrapper.scrollWidth > wrapper.clientWidth ? "block" : "none";
    }

    if (topScroll.dataset.synced !== "1") {
      ...
      topScroll.dataset.synced = "1";
    }

    refreshTopScrollWidth();
    topScroll.scrollLeft = wrapper.scrollLeft;
  });
}

function refreshTopHorizontalScrollbarsSoon() {
  requestAnimationFrame(() => {
    ensureTopHorizontalScrollbars();
    setTimeout(ensureTopHorizontalScrollbars, 80);
  });
}

document.addEventListener("DOMContentLoaded", refreshTopHorizontalScrollbarsSoon);
window.addEventListener("resize", refreshTopHorizontalScrollbarsSoon);

const originalRenderGroupedModuleForTopScroll = renderGroupedModule;
renderGroupedModule = function(module, rowRenderer) {
  originalRenderGroupedModuleForTopScroll(module, rowRenderer);
  refreshTopHorizontalScrollbarsSoon();
};

const originalApplyTableScaleForTopScroll = applyTableScale;
applyTableScale = function(module) {
  originalApplyTableScaleForTopScroll(module);
  refreshTopHorizontalScrollbarsSoon();
};


/* =========================
   TOP + BOTTOM SCROLL SYNC
========================= */
const syncedTopScrollbars = new Map();

function setupDualScrollbars() {
  document.querySelectorAll(".table-wrapper").forEach(wrapper => {
    let topScroller = syncedTopScrollbars.get(wrapper);

    if (!topScroller) {
      topScroller = document.createElement("div");
      topScroller.className = "top-scrollbar";

      const topInner = document.createElement("div");
      topInner.className = "top-scrollbar-inner";

      topScroller.appendChild(topInner);
      wrapper.parentNode.insertBefore(topScroller, wrapper);
      ...
      syncedTopScrollbars.set(wrapper, topScroller);

      const observer = new ResizeObserver(() => {
        syncDualScrollbar(wrapper, topScroller);
      });
...
function syncDualScrollbar(wrapper, topScroller) {
  const table = wrapper?.querySelector("table");
  const topInner = topScroller?.querySelector(".top-scrollbar-inner");

  if (!table || !topInner) return;

  const needsScroll = table.scrollWidth > wrapper.clientWidth + 2;

  topScroller.style.display = needsScroll ? "block" : "none";
...
const originalRenderProjectsForTopScroll = renderProjects;
renderProjects = function() {
  originalRenderProjectsForTopScroll();
  requestAnimationFrame(setupDualScrollbars);
};

const originalRenderPipelineForTopScroll = renderPipeline;
renderPipeline = function() {
  originalRenderPipelineForTopScroll();
  requestAnimationFrame(setupDualScrollbars);
};

const originalRenderDeploymentForTopScroll = renderDeployment;
renderDeployment = function() {
  originalRenderDeploymentForTopScroll();
  requestAnimationFrame(setupDualScrollbars);
};

const originalRenderApplicationForTopScroll = renderApplication;
renderApplication = function() {
  originalRenderApplicationForTopScroll();
  requestAnimationFrame(setupDualScrollbars);
};

const originalRenderTimelineForTopScroll = renderTimeline;
renderTimeline = function() {
  originalRenderTimelineForTopScroll();
  requestAnimationFrame(setupDualScrollbars);
};

window.addEventListener("resize", setupDualScrollbars);

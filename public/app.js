...
/* =========================
   EXPORTS - REAL XLSX
   Uses ExcelJS in the browser instead of HTML/XML fake Excel.
========================= */
...
function styleForExcelCell(domCell) {
  ...
  return {
    alignment: {
      horizontal: computed.textAlign === "start" ? "left" : computed.textAlign,
      vertical: "top",
      wrapText: true
    },
    font: {
    ...
    fill: {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: cssColorToArgb(computed.backgroundColor) || "FFFFFFFF" }
    },
    border: {
  ...
  function walk(currentNode, parentStyle) {
    if (currentNode.nodeType === Node.TEXT_NODE) {
      ...
      runs.push({
        ...
        font: {
          name: "Arial",
          size: parentStyle.size || 11,
          bold: parentStyle.bold || false,
          color: { argb: parentStyle.color || "FF000000" }
        }
      });
      return;
    }
...
function getExportRowsFromCurrentDom(module, groupTitle) {
  const cfg = moduleConfig(module);
  if (!cfg) return [];

  const originalTable = document.getElementById(cfg.tableId);
  ...
  allRows.forEach(row => {
    const headerCell = row.querySelector(".group-title-edit");

    if (headerCell) {
      const currentGroup = normalizeGroupName(headerCell.innerText);
      insideTarget = currentGroup === safeGroup;

      if (insideTarget) rows.push(row);
      return;
    }
...
async function exportGroup(module, groupTitle) {
  const rows = getExportRowsFromCurrentDom(module, groupTitle);

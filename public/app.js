...
function excelCellValueFromDom(domCell) {
  ...
  const plainText = domCell.innerText || "";

  if ([...plainText].some(char => isCellIconChar(char))) {
    return {
...
function applyDomRowToWorksheetRow(excelRow, domRow) {
  ...
  domCells.forEach((domCell, index) => {
    ...
    excelCell.font = [...(domCell.innerText || "")].some(char => isCellIconChar(char))

...
/* =========================================================
   TIMELINE 31-DAY CALENDAR PATCH
   - Timeline no longer uses group pagination
   - Every Timeline group has max 31 rows = Day 1-31
   - Invalid dates are disabled automatically, e.g. Feb 29/30/31, Apr 31
   - Refresh button can stay in HTML; CSS/index polish supplied separately
========================================================= */
...
function timelineDayForRow(row, groupRowsList) {
  const sorted = [...groupRowsList].sort((a, b) => a.id - b.id);
...
// Timeline rows now save project_group and always keep 12 month cells.
saveTimelineRow = async function(id) {
...
updateTimelineCell = function(id, monthIndex, value) {
  const row = timelineData.find(item => Number(item.id) === Number(id));
  if (!row) return;
...
updateTimelineStatus = function(id, value) {
  const row = timelineData.find(item => Number(item.id) === Number(id));
  if (!row) return;
  ...
  return `
    <tr class="timeline-day-row">
      ...
      ${months.map((m, monthIndex) => {
        ...
        return `
          <td
            class="timeline-month-cell ${editable ? "" : "timeline-disabled-cell"}"
            ${editable ? "contenteditable=\"true\"" : "contenteditable=\"false\""}
            data-fill="${editable ? "true" : "false"}"
            data-module="timeline"
            data-id="${row.id}"
            data-field="${monthIndex}"
            ${disabledText}
            ${editable ? `onblur="updateTimelineCell(${row.id}, ${monthIndex}, this.innerText)"` : ""}
          >${editable ? escHtml(m || "") : "—"}</td>
        `;
      }).join("")}
      <td contenteditable="true" data-fill="true" data-module="timeline" data-id="${row.id}" data-field="status" onblur="updateTimelineStatus(${row.id}, this.innerText)">${escHtml(row.status || "")}</td>
    </tr>`;
...
// Timeline has no group pagination anymore; one group = max 31 visible day rows.
const previousGroupPaginationHtmlTimeline31 = groupPaginationHtml;
groupPaginationHtml = function(module, groupTitle, rowCount) {
  if (module === "timeline") {
    return `<span class="group-page-note timeline-day-limit-note">${rowCount}/31 day rows</span>`;
  }
  return previousGroupPaginationHtmlTimeline31(module, groupTitle, rowCount);
};

// Make Timeline Add Row stop at 31 days.
const previousAddRowToGroupTimeline31 = addRowToGroup;
addRowToGroup = async function(module, group) {
  if (module !== "timeline") return previousAddRowToGroupTimeline31(module, group);

  const cfg = moduleConfig("timeline");
  group = normalizeGroupName(group);

  if (!group || group === "Untitled") {
    alert("Please rename this Timeline group before adding rows");
    return;
  }

  const currentCount = getTimelineGroupRows(group).length;
  if (currentCount >= 31) {
...
// Make Add Group for Timeline create day 1 only and use the selected year.
const previousAddGroupTimeline31 = addGroup;
addGroup = async function(module) {
  if (module !== "timeline") return previousAddGroupTimeline31(module);

  const cfg = moduleConfig("timeline");
  const group = getGroupInputValue("timeline");
  ...
  const exists = timelineData.some(row => normalizeGroupName(row.project_group) === group);
  if (exists && !confirm("This Timeline group already exists. Add one more day row inside it?")) return;

  if (exists && getTimelineGroupRows(group).length >= 31) {
    alert("Timeline groups are limited to 31 day rows.");
    return;
  }
  ...
  clearGroupInput("timeline");
  await cfg.load();
};

// Timeline export now includes Day column and marks invalid dates as NA.
const previousExportGroupTimeline31 = exportGroup;
exportGroup = function(module, groupTitle) {
  if (module !== "timeline") return previousExportGroupTimeline31(module, groupTitle);

  const rows = rowNumberedGroups(timelineData, "timeline")
    .find(group => normalizeGroupName(group.title) === normalizeGroupName(groupTitle))?.rows || [];

  if (!rows.length) return alert("No rows to export for this Timeline group");

  const headers = ["Day", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Status"];
  const csvRows = [...rows]
  ...
  exportCsv(`timeline_${safeFilename(groupTitle)}_${timelineYearValue()}.csv`, headers, csvRows);
};

// Timeline group renderer: no page slicing, fixed max 31 visible rows.
const previousRenderGroupedModuleTimeline31 = renderGroupedModule;
renderGroupedModule = function(module, rowRenderer) {
  if (module !== "timeline") return previousRenderGroupedModuleTimeline31(module, rowRenderer);

  const cfg = moduleConfig("timeline");
  const col = document.querySelector(cfg.selectCol);
  if (col) col.style.display = selectMode.timeline ? "" : "none";

  const table = document.getElementById(cfg.tableId);
  table.innerHTML = "";

  const groups = rowNumberedGroups(timelineData, "timeline");

  if (!groups.length) {
    table.innerHTML = `
      <tr class="group-title-row">
        <td colspan="99">No Timeline groups yet. Type a title above and click Add Group.</td>
      </tr>
    `;
    clearGroupedPagination("timeline");
    return;
  }
  ...
  groups.forEach(group => {
    ...
    if (!isGroupCollapsed("timeline", group.title)) {
      rows.forEach(row => { table.innerHTML += rowRenderer(row); });
    }
  });
...
// Fill handle should ignore disabled Timeline dates.
document.addEventListener("mousedown", (e) => {
  const cell = e.target.closest("td.timeline-disabled-cell");
  if (cell) {
    e.preventDefault();
    e.stopPropagation();
  }
...
/* =========================================================
   MODULE-LEVEL PAGINATION PATCH
   - Dashboard/Pipeline/Deployment/Application: 90 visible rows per module page
   - Timeline: 93 visible rows per module page
   - Existing group pagination remains unchanged
...
function isModulePaged(module) {
  return Object.prototype.hasOwnProperty.call(MODULE_PAGE_LIMITS, module);
}

function getModuleSectionPage(module) {
  return Math.max(1, Number(moduleSectionPageState[module]) || 1);
}
...
function groupVisibleContribution(module, group) {
  if (module === "timeline") return Math.min(group.rows.length, 31);
  return Math.min(group.rows.length, GROUP_ROWS_PER_PAGE);
}
...
function getModulePagedGroups(module) {
  ...
  return rowNumberedGroups(cfg.data(), module).map(group => {
    ...
    return {
      ...group,
      modulePage: assignedPage,
      visibleContribution: contribution
    };
  });
}
...
function renderModulePagination(module) {
  if (!isModulePaged(module)) return;

  const div = ensureModulePaginationElement(module);
  if (!div) return;

  const totalPages = getModuleTotalPages(module);
  let current = Math.min(getModuleSectionPage(module), totalPages);
...
function placeNewGroupInModuleOrder(module, groupTitle, targetPage) {
  if (typeof getStoredGroupOrder !== "function" || typeof setStoredGroupOrder !== "function") return;
...
const previousAddGroupModulePagination = addGroup;
addGroup = async function(module) {
  if (!isModulePaged(module)) return previousAddGroupModulePagination(module);
  ...
  if (usedRows > 0 && usedRows + contribution > limit) {
    targetPage += 1;
  }
...
// Final renderer override: show only groups assigned to the current module page.
renderGroupedModule = function(module, rowRenderer) {
  const cfg = moduleConfig(module);
  if (!cfg) return;
  ...
  const totalPages = getModuleTotalPages(module);
  let currentModulePage = Math.min(getModuleSectionPage(module), totalPages);
  ...
  if (!groups.length) {
    table.innerHTML = `
      <tr class="group-title-row">
        <td colspan="99">No groups on this page yet. Type a title above and click Add Group.</td>
      </tr>
    `;
    clearGroupedPagination(module);
...
// Keep final renderer bindings after module pagination override.
renderProjects = function() { renderGroupedModule("dashboard", renderDashboardRow); };
renderPipeline = function() { renderGroupedModule("pipeline", renderPipelineRow); };
renderDeployment = function() { renderGroupedModule("deployment", renderDeploymentRow); };
renderApplication = function() { renderGroupedModule("application", renderApplicationRow); };
...
// Reset module page only when user refreshes a module intentionally.
const previousRefreshDashboardModulePagination = refreshDashboard;
refreshDashboard = function() { setModuleSectionPage("dashboard", 1); previousRefreshDashboardModulePagination(); };
const previousRefreshPipelineModulePagination = refreshPipeline;
refreshPipeline = function() { setModuleSectionPage("pipeline", 1); previousRefreshPipelineModulePagination(); };
const previousRefreshDeploymentModulePagination = refreshDeployment;
refreshDeployment = function() { setModuleSectionPage("deployment", 1); previousRefreshDeploymentModulePagination(); };
const previousRefreshApplicationModulePagination = refreshApplication;
refreshApplication = function() { setModuleSectionPage("application", 1); previousRefreshApplicationModulePagination(); };
const previousRefreshTimelineModulePagination = refreshTimeline;
refreshTimeline = function() { setModuleSectionPage("timeline", 1); previousRefreshTimelineModulePagination(); };
